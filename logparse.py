...
class MainFrame(wx.Frame):
	def __init__(self, parent, title):
		...
		self.st = wx.StaticText(panel, -1, 'Enter Your Character Name', (5,53))
				...
				if logitem.startswith("20::"):
					...
					if logitem.find("your") != -1 or logitem.find("Your") != -1:
						...
						if monster == currentmonster["monster"]:						
							...
							if damage:
								currentmonster["damage"].append([damage, critical])
					...
					if monster == currentmonster["monster"]:						
						...
						damage = logitem[logitem.find("for ") +4:logitem.find(" points")]
						if damage:
							currentmonster["otherdamage"].append([damage, critical])
...
def uploadToDB():
	...
	if response == "" or response.upper() == "Y" or response.upper() == "YES":
		...
		print "Total Global Battle Records: %s" % url["totalbattlerecords"]
		print "Records Imported (Duplicates ignored): %s" % url["recordsimported"]
		print "Records Updated: %s" % url["updatedrecords"]
	else:
